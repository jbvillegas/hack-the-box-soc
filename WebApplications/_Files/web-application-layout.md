...
For example, an individual web application may have all of its core functionality securely implemented. However, due to a lack of proper access control measures in its design, i.e., use of [Role-Based Access Control(RBAC)](https://en.wikipedia.org/wiki/Role-based_access_control), users may be able to access some admin features that are not intended to be directly accessible to them or even access other user's private information without having the privileges to do so. To fix this type of issue, a significant design change would need to be implemented, which would likely be both costly and time-consuming.
...
This section explains **Web Application Layout**. It focuses on Web Application Infrastructure, Web Application Components, Web Application Architecture, Architecture Security.
...
I learned the basic ideas behind **Web Application Layout** and how they can help me investigate and respond to security activity.
