...
Command prompt showing SharpHound execution with collection methods and status updates. Enumeration completed successfully on 19/12/2022.
```
...
Diagram showing Bob with GenericAll permissions over Anni and Server01.

Bob has full rights over the user Anni and the computer Server01. Below is what Bob can do with each of these:

- **Case 1: Full rights over the user Anni.** Bob can modify the object Anni by specifying a bonus SPN value and then perform the Kerberoast attack against it. Bob can also modify Anni's password and log in as that account, directly inheriting everything Anni can do. If Anni is a Domain Admin, Bob would have the same rights.
- **Case 2: Full control over the computer object Server01.** If LAPS is used in the environment, Bob can obtain the password stored in the attributes and authenticate as the local Administrator account to this server. Another escalation path is abusing Resource-Based Kerberos Delegation, allowing Bob to authenticate as anyone to this server. Server01 is trusted for Unconstrained Delegation, so administrative rights on this server could allow Bob to compromise the identity of a Domain Controller or another sensitive computer object.
...
- Automate as much as possible from the access management process, and only assign privileged access to administrative accounts. This ensures that administrators do not manually edit accounts, reducing the risk of introducing delegated rights to unprivileged users.
...
Event 4738: Bob modified Anni's account with no details of changes.
...
Event 4742: Bob modified Server01 with no details of changes.
...
- Assign relatively high ACLs to a user account used as a honeypot via a previously discussed technique, such as a user whose fake credentials are exposed in the description field. Having ACLs assigned to the account may provoke an adversary to verify whether the account's exposed password is valid because it holds high potential.
- Have an account that everyone or many users can modify. This user should be a honeypot user with some activity to mimic real users. Any changes occurring on this honeypot user are malicious because there is no valid reason for anyone to perform actions on it, except administrators who may occasionally need to reset the account's password to make it look realistic. Therefore, any event ID 4738 associated with the honeypot user should trigger an alert. Mature organizations may also immediately disable the user performing the change and initiate a forensic investigation on the source device. For completeness, in the Bob/Anni example, a change to Anni's account would be detected as follows:
Event 4738: Bob modified Anni's account with no details of changes.
...
This section explains how misconfigured Active Directory object ACLs can provide privilege escalation paths through user, computer, and group permissions. It focuses on identifying, preventing, detecting, and monitoring ACL abuse.

## What I Did

I reviewed the Hack The Box material, followed the SharpHound and BloodHound example, and reviewed the prevention, detection, and honeypot guidance shown in this section.

## What I Learned

I learned how excessive ACL permissions can enable attacks such as password modification, Kerberoasting, LAPS password access, and Resource-Based Kerberos Delegation, along with ways to detect suspicious object changes.
