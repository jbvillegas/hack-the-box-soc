...
- Resetting a user password does not invalidate the certificate. With certificates, it does not matter how many times a user changes their password; the certificate will still be valid (unless expired or revoked).
...
Vulnerable certificate template details: UserCert, valid for 10 years, used for client authentication, with enrollment rights for Domain Admins and Users.
```
...
- The certificate can be used for `Client Authentication`, so we can use it for login/authentication.
...
Certify.exe request: Bob requests a certificate as Administrator; certificate issued.
```
...
```shellsession
villegasjb@htb[/htb]$ sed -i 's/\s\s\+/\n/g' cert.pem
```

Then we can execute the openssl command mentioned in the output of Certify.

```shellsession
villegasjb@htb[/htb]$ openssl pkcs12 -in cert.pem -keyex -CSP "Microsoft Enhanced Cryptographic Provider v1.0" -export -out cert.pfx
```

OpenSSL command to export cert.pem to cert.pfx with password prompt.
...
Rubeus command requests TGT for Administrator using cert.pfx; ticket successfully imported.
```
...
Command prompt showing directory listing of \dc1\c$ with folders like DFSReports and Mimikatz.
```
...
Events 4886 and 4887: Bob requested and received a certificate; no SAN details provided.
...
Certificate request by EAGLE\bob for UserCert, effective 10/16/2022, expires 10/16/2024.
...
Certificate details showing Subject Alternative Name as Principal Name=Administrator.
...
Certificate request by Bob for Administrator using UserCert template from WS001.
...
Event 4768: Administrator requested Kerberos TGT from IP 172.16.18.25; certificate details included.
...
This section explains how the AD CS ESC1 misconfiguration can allow a user to request a certificate for another account and use it to obtain a Kerberos TGT. It focuses on the attack path, prevention, and detection through certificate and authentication events.

## What I Did

I reviewed the Hack The Box material, followed the Certify, OpenSSL, and Rubeus attack flow, and reviewed the prevention and detection guidance shown in this section.

## What I Learned

I learned how insecure certificate templates can enable privilege escalation through SAN control, how certificates can be converted and used for PKINIT authentication, and how CA events can help identify abuse.